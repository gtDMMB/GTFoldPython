...
gomp_test_lock_30 (omp_lock_t *lock)
{
  //int oldval = 0;
  omp_lock_t oldval;
  memset(&oldval, 0, sizeof(omp_lock_t));
  return __atomic_compare_exchange_n ((int*) lock, (int *) &oldval, 1, false,
...
gomp_test_nest_lock_30 (omp_nest_lock_t *lock)
{
  ...
  oldval = 0;
  if (__atomic_compare_exchange_n ((int*) &lock->lock, (int*) &oldval, 1, false,
				   MEMMODEL_ACQUIRE, MEMMODEL_RELAXED))
    {
      lock->owner = me;
      lock->count = 1;
