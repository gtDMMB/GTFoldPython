...
class AdvancedDouble_Native{
	private:
		double value;
	public:
		inline char * getDoubleValue() const { 
		     char dblstr[64];
		     sprintf(dblstr, "%.20f", value);
		     return strdup(dblstr);
		}
...
class AdvancedDouble_BigNum{
	...
	public:
		inline char * getDoubleValue() const {
                     char dblstr[64];
		     if(bigValue != 0) gmp_printf("%.*Ff", PRINT_DIGITS_AFTER_DECIMAL, *bigValue);
                     else              return strdup("0.0");
		     return strdup(dblstr);
		}
...
class AdvancedDouble_BigNumOptimized{
	...
	public:
		inline char * getDoubleValue() const { 
		     char dblstr[64];	
	             gmp_sprintf(dblstr, "%.*Ff", PRINT_DIGITS_AFTER_DECIMAL, bigValue);
                     return strdup(dblstr);
		}
...
class AdvancedDouble_Hybrid{
	...
	public:
		inline char * getDoubleValue() const { 
		      char dblstr[64];	
		      if(isBig == 'y') gmp_sprintf(dblstr, "%.*Ff", PRINT_DIGITS_AFTER_DECIMAL, *bigValue);
                      else if(isBig=='n') sprintf(dblstr, "%f", *smallValue);
                      else sprintf(dblstr, "Unknown isBig = %c\n", isBig);
		      return strdup(dblstr);
		}
