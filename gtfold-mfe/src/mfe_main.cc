...
void init_fold(const char* seq) {
  ...
  if (CONS_ENABLED) {
    init_constraints(constraintsFile, len);
  }

  if (SHAPE_ENABLED) {
    readSHAPEarray(shapeFile,len);
  }
...
int mfe_main(int argc, char** argv) {
	...
	// Read in thermodynamic parameters. Always use Turner99 data (for now)
        readThermodynamicParameters(paramDir, PARAM_DIR, UNAMODE, RNAMODE, T_MISMATCH);
	...
	t1 = get_seconds();
	trace(seq.length(), print_energy_decompose, energyDecomposeOutFile);
	t1 = get_seconds() - t1;
	...
	save_ct_file(outputFile, seq.c_str(), energy);
